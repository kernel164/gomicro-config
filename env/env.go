		...
		for i, k := range keys {
			if i == 0 {
				if intValue, err := strconv.Atoi(value); err == nil {
					...
					if e.valueResolver == nil {
						tmp[k] = value
					} else {
						val, err := e.valueResolver(value)
						if err != nil {
							return nil, err
						}
						tmp[k] = val
...
func NewSource(opts ...source.Option) source.Source {
	...
	vr, _ := options.Context.Value(valueResolverKey{}).(ValueResolver)
	return &env{prefixes: pre, strippedPrefixes: sp, valueResolver: vr, opts: options}
}
